	...
	var app = {
		...
		socketActions: function(){
			 ...
			 socket.on('edited', function(data){
			 	$('li#'+data._id+' .view label').html(data.content);
			 	$('li#'+data._id+' input.edit').val(data.content);
			 });
		...
		list: function(){
			socket.on('all', function(data){
				...
				for(var i = 0; i< data.length; i++){
					if(data[i].isDone){
						$('#todo-list').append('<li id="'+data[i]._id+'" class="completed"><div class="view"><input data-todoId="'+data[i]._id+'" class="toggle" type="checkbox" checked><label>'+data[i].content+'</label><button data-todoId="'+data[i]._id+'" class="destroy"></button></div><input data-todoId="'+data[i]._id+'" class="edit" value="'+data[i].content+'"></li>');
					}else{
						$('#todo-list').append('<li id="'+data[i]._id+'"><div class="view"><input data-todoId="'+data[i]._id+'" class="toggle" type="checkbox"><label>'+data[i].content+'</label><button data-todoId="'+data[i]._id+'" class="destroy"></button></div><input data-todoId="'+data[i]._id+'" class="edit" value="'+data[i].content+'"></li>');
					}
				}
		...
		addToList: function(new_todo){
			$('#todo-list').append('<li id="'+new_todo._id+'"><div class="view"><input data-todoId="'+new_todo._id+'" class="toggle" type="checkbox"><label>'+new_todo.content+'</label><button data-todoId="'+new_todo._id+'" class="destroy"></button></div><input data-todoId="'+new_todo._id+'" class="edit" value="'+new_todo.content+'"></li>');
			app.mainSectionToggle();
		},
